 ...
 */
const getUsersByGender = (arr, property) =>
  arr.filter (x => x.gender === property)
      .map (y => y.name);
...
const getUsersWithAge = (arr, min, max) => 
  arr.filter (x => x.age > min && x.age < max);
...
const getTotalBalance = (arr) => 
  arr.reduce ((acc,x) => acc + x.balance, 0);
...
const getUsersByFriend = (arr, name) =>
  arr.filter (x => x.friends.includes (name))
      .map (y => y.name);
