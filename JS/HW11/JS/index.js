...
reset.addEventListener('click', () => {
  cards.innerHTML = template(laptops)
})
...
function filterCards(evt) {
  evt.preventDefault();
  let filter = getArrayCheckedInputs()
    .reduce((acc, elem) => {
      acc[elem.name] ?
...
function getFiltered(fil, listOfObjs) {
  return Object.keys(fil).reduce((ac, el) => {
    return ac.filter(e => fil[el].includes('' + e[el]))
  }, listOfObjs)
}